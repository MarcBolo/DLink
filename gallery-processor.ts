        ...
        images.forEach((imgSource, index) => {
...

            {
                ...
                if (mediaSrc) {
                    ...
                    // render 会执行后处理器，渲染完成由返回的 Promise 给出：用它取代原先固定的 50ms 定时器，
                    // 避免内容晚于定时器出现时媒体样式漏加
                    void MarkdownRenderer.render(plugin.app, imgSource, item, ctx.sourcePath, plugin as unknown as import('obsidian').Component)
                        .then(() => {
