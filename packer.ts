...
/** 遍历时跳过的目录；配置目录名由用户决定，只能从 Vault#configDir 读取，不能写死 */
function getSkipDirs(app?: App): Set<string> {
  ...
  if (configDir) skipDirs.add(configDir);
  return skipDirs;
}
  ...
  for (const regex of [imgRegex, linkRegex, mediaRegex]) {
    ...
    while ((match = regex.exec(content)) !== null) {
      ...
      if (fs.existsSync(destPath)) {
        ...
        existingVaultPath = findFileRecursive(attachmentsDir, fileName, 5, plugin.app) || '';
      }
...
export async function packOut(plugin: IDualLinkPlugin): Promise<void> {
  ...
  while ((match = internalLinkRegex.exec(content)) !== null) {
    ...
    if (fs.existsSync(destFullPath)) {
      ...
      existingExtPath = findFileRecursive(externalDir, fileName, 5, plugin.app) || '';
    }
